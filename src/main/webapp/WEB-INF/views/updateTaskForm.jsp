...
<head>
  ...
  <title>DevSync - Update Task</title>
...
<section class="flex justify-center items-center min-h-screen px-4">
  <div class="w-full max-w-3xl p-8 bg-white rounded-lg shadow dark:bg-gray-800 lg:px-12">
    <h2 class="text-3xl font-semibold text-gray-800 capitalize dark:text-white mb-6">Update Task</h2>
    <form class="grid grid-cols-1 gap-6" action="tasks?action=update" method="post">
        ...
        <div>
          ...
          <select name="assignee_id" id="assignee_id" class="js-example-basic-single block w-full px-4 py-2 mt-2 text-gray-800 bg-gray-100 border border-gray-300 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
            <c:forEach var="user" items="${users}">
              <option value="${user.id}" ${task.assignee.id == user.id ? 'selected' : ''}>${user.firstName} ${user.lastName}</option>
            </c:forEach>
          </select>
        </div>
      </c:if>
      ...
      <div>
        ...
        <select id="tags" class="js-example-basic-multiple block w-full px-4 py-2 mt-2 text-gray-800 bg-gray-100 border border-gray-300 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500" name="tags[]" multiple="multiple">
          <c:forEach var="tag" items="${tags}">
            <option value="${tag.id}" ${task.tags.contains(tag) ? 'selected' : ''}>${tag.name}</option>
          </c:forEach>
        </select>
      </div>
